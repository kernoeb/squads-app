...
object OAuthConfig {
    ...
    fun deviceCodeUrl(): String = "https://login.microsoftonline.com/$TENANT/oauth2/devicecode"

    /** OAuth v1 token endpoint for polling device code (same as CLI) */
    fun tokenUrl(): String = "https://login.microsoftonline.com/$TENANT/oauth2/token"

    /** OAuth v2 token endpoint for refresh token renewal */
    fun tokenV2Url(): String = "https://login.microsoftonline.com/$TENANT/oauth2/v2.0/token"

    /** Device code request body — resource=skype API (same as CLI) */
    fun deviceCodeBody(): String = "client_id=$CLIENT_ID&resource=https://api.spaces.skype.com"
